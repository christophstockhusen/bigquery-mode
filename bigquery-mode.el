...
(defun bqm-list-projects ()
  ...
  (let ((buf (get-buffer-create bqm-projects-buffer-name))
    ...
    (with-current-buffer buf
      ...
      (tabulated-list-mode))
    (let ((height (max 40 (min 10 (+ (length project-list) 2)))))
...
(defun bigquery-show-datasets ()
  ...
  (let ((buf (get-buffer-create bigquery-datasets-buffer-name))
    ...
    (with-current-buffer buf
      ...
      (tabulated-list-mode))
    (let ((height (max 40 (min 10 (+ (length dataset-list) 2)))))
      (display-buffer-at-bottom buf '((window-height . fit-window-to-buffer))))
    (let ((w (get-buffer-window buf)))
      (select-window w))))
...
(defun bigquery-show-tables (dataset-name)
  ...
  (let ((buf (get-buffer-create bigquery-tables-buffer-name))
    ...
    (with-current-buffer buf
      ...
      (tabulated-list-mode))
    (let ((height (max 40 (min 10 (+ (length table-list) 2)))))
...
(defun bigquery-show-schema (table-name)
  ...
  (let ((buf (get-buffer-create bigquery-schema-buffer-name))
    ...
    (with-current-buffer buf
      ...
      (tabulated-list-mode))
    (let ((height (max 40 (min 10 (+ (length schema-list) 2)))))
...
(defun bqm-format-bytes (bytes)
  (let* ((units ["B" "KB" "MB" "GB" "TB" "PB"])
    ...
    (while (and (<= counter (length units))
      ...
      (setq small (/ bytes (expt 1000 counter))))
...
(defun bqm-show-table (table-name)
  ...
  (let ((buf (get-buffer-create bigquery-table-buffer-name))
    ...
    (with-current-buffer buf
      ...
      (tabulated-list-mode))
    (let ((height (max 40 (min 10 (+ (length content) 2)))))
      (display-buffer-same-window buf '((window-height . fit-window-to-buffer))))
    (let ((w (get-buffer-window buf)))
      (select-window w))))
